...
%%
figure(1);
set(gcf,'Position',[100,0,800,500]);
...
figure(1);
plot(linspace(0,100,StepNum),StanceSeq,'--','LineWidth',2);
hold on;
for ii = StartNum : 15 : EndNum
    if StanceSeq(ii-StartNum+1) == 2
        scatter((ii-StartNum)*100/(EndNum-StartNum),2,'b','d','DisplayName','Flat Foot','LineWidth',1.5);
    elseif StanceSeq(ii-StartNum+1) == 3
        scatter((ii-StartNum)*100/(EndNum-StartNum),3,'k','^','DisplayName','Heel Off','LineWidth',1.5);
    elseif StanceSeq(ii-StartNum+1) == 4
        scatter((ii-StartNum)*100/(EndNum-StartNum),4,'m','+','DisplayName','Swing','LineWidth',1.5);
    end 
end
hold on;
for ii = StartNum : 1 : EndNum
    if StanceSeq(ii-StartNum+1) == 1
    scatter((ii-StartNum)*100/(EndNum-StartNum),1,'r','o','DisplayName','Heel Strike','LineWidth',1.5);
    end
end
hold off;
...
title('\fontsize{15}Walking Sequence of Foot Drop Gait with Neurological Disorder');
