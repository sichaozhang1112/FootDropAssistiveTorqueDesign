...
% [ Fgx(ii,:) ] = GroundReactionForceX( t(ii),x(ii,:)',theta(ii,:)',xf(ii,:),xf_dot(ii,:),yf(ii,:),yf_dot(ii,:) );
...
[ Tmr(ii,:),Tmi(ii,:) ] = MuscleTorque( theta(ii,:)',theta_dot(ii,:)',u(ii,:)',Sron(ii,:),Sroff(ii,:),Slon(ii,:),Sloff(ii,:),Srst(ii,:),Slst(ii,:) );
...
[ Tm17(ii,:),u13(ii,:) ] = EstimateTm17( phi(ii,:),Sg(ii,:),Slon(ii,:),Sloff(ii,:),Slst(ii,:),theta_dot(ii,:)' );
...
for ii = 1 : size(TorqueNum,2)
    subplot(7,1,ii);
    plot((1:EndNum),Tm(1:EndNum,TorqueNum(ii)));
    xlabel('step');
